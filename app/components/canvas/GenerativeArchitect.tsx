...
const SymmetricalArt = ({ controls }: { controls: any }) => {
  ...
  // 1. 获取当前画布的宽高比，用于计算视口边界
  const { size } = useThree();
  const aspect = size.width / size.height;

  // 2. 预计算摄像机视锥体参数 (FOV 45度，Z位置 80)
  ...
  const gridData = useMemo(() => {
    const countX = 80; 
    const countZ = 15;
    const maxCount = countX * countZ * 2;
    ...
    for(let x = 0; x < countX; x++) {
      for(let z = 0; z < countZ; z++) {
        ...
        // --- 视区裁剪检查 (Grid) ---
        const dist = cameraZ - posZ;
        const maxVisibleY = dist * tanHalfFov + 2.0; 
        const maxVisibleX = maxVisibleY * aspect + 2.0;
        
        if (posX > maxVisibleX) continue;
    ...
    return { count: i, offsets: offsets.slice(0, i * 3) };
  }, [controls.gridDensity, aspect]); 
  ...
  const blockData = useMemo(() => {
    ...
    for (let j = 0; j < quadrantCount; j++) {
      ...
      const maxVisibleY = dist * tanHalfFov + (controls.blockSize * 10.0); 
      const maxVisibleX = maxVisibleY * aspect + (controls.blockSize * 10.0);

      if (x > maxVisibleX || y > maxVisibleY) {
          continue; 
      }
    ...
    return { 
      count: i, 
      offsets: offsets.slice(0, i * 3), 
      dataArray: dataArray.slice(0, i * 4), 
      scaleMap: scaleMap.slice(0, i * 2) 
    };
  }, [controls.spreadX, controls.spreadY, controls.depthZ, controls.blockSize, aspect]);
  ...
  return (
    <group>
      <instancedMesh ref={gridMeshRef} args={[undefined, undefined, gridData.count]} count={gridData.count}>
      ...
      <instancedMesh ref={blockMeshRef} args={[undefined, undefined, blockData.count]} count={blockData.count}>
